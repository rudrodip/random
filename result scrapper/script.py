...
OBJECT_KEYS = ['name','id','bangla','english','ict', 'physics', 'chemistry', 'main_subject', '4th_subject', 'total']
...
def getObject(html):
    ...
    try:
        row = [int(marks[4].text), int(marks[10].text), int(marks[20].text), int(marks[27].text), int(marks[34].text), int(marks[41].text), int(marks[48].text)]
    except:
        row = [0, 0, 0, 0, 0, 0, 0]
